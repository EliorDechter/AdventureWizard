...
void do_editor(WzGui* gui, PlatformTargetTexture target_texture, wzrd_icons icons, WzStr* debug_str, WzWidget* target_panel_out) {
	(void)icons;
	static bool finish;
		...
		{
			{
				...
				if (1)
				{
					...
					WzWidget row = editor_hpanel(form);
					...
					{
						WzWidget buttons_panel = editor_hpanel(dialog);
						{
							bool b1;

...
Crate* EguiGetCurrentWindow() {
	Crate* result = &wz->crates_stack[wz->current_crate_index];
...
Crate* EguiGetPreviousWindow() {
	...
	Crate* result = &wz->crates_stack[wz->current_crate_index - 1];

	return result;
}

WzWidgetData* wzrd_box_get_parent() {
	if (wz->crates_stack[wz->current_crate_index].box_stack_count < 2)
	{
		return &wz->widgets[0];
	}
	int current_box_index = wz->crates_stack[wz->current_crate_index].box_stack_count - 2;
	int final_index = wz->crates_stack[wz->current_crate_index].box_stack[current_box_index];
	WzWidgetData* result = &wz->widgets[final_index];
...
int wzrd_compare_boxes(const void* element1, const void* element2) {
	...
	WzWidgetData* c2 = wz_widget_get(index1);
...
void wz_widget_add_rect_new(WzWidget widget, int x, int y, unsigned w, unsigned h, unsigned color)
{
	WzWidgetItem item = { 0 };
	item.x = x;
	item.y = y;
...
void wzrd_crate_begin(int layer, WzWidgetData box) {
	...
	wz->total_num_windows++;
	wz->current_crate_index++;

	Crate* current_window = EguiGetCurrentWindow();

	// Set new window
	*current_window = (Crate){ .layer = layer, .index = wz->current_crate_index, };
	...
	bool enable_input)
{
	...
	wz->mouse_pos = (wzrd_v2){ mouse_x, mouse_y };
	...
	// Stylesheet
	wz->stylesheet.label_color = WZ_LIGHTGRAY;
	wz->stylesheet.label_item_selected_color = WZ_LIGHTGRAY;
...
#define TEMP_BUFFER_SIZE 128
	...
	for (int i = 0; i < MAX_NUM_WIDGETS; ++i)
	{
		...
		// Draw Widget background - use temp buffer
		temp_buffer_count = 0;
		wz_draw_rect(&temp_buffer[temp_buffer_count++],
			(WzRect) {
			.x = widget.actual_x,
				.y = widget.actual_y,
				.w = widget.actual_w,
		...
		buffer->commands[buffer->count] = temp_buffer[0];
		buffer->commands[buffer->count].widget_index = widget.handle.handle;
		buffer->count++;

		for (int i = 0; i < MAX_NUM_WIDGETS; ++i) {
			WzWidget widget = wz->widgets[i].handle;
			WzWidgetData* widget_data = wz_widget_get(widget);

			// Borders
			if (!(widget_data->actual_w <= 2 || widget_data->actual_h <= 2)) {
				if (widget_data->border_type != WZ_BORDER_TYPE_NONE) {
				...
				WzRect top0 = (WzRect){ 0, 0, widget_data->actual_w - line_size, line_size };
				WzRect left0 = (WzRect){ 0, 0, line_size, widget_data->actual_h };
				WzRect top1 = (WzRect){ 0 + line_size, 0 + line_size, widget_data->actual_w - 3 * line_size, line_size };
				WzRect left1 = (WzRect){ 0 + line_size, 0 + line_size, line_size, widget_data->actual_h - line_size };
				WzRect bottom0 = (WzRect){ 0, 0 + widget_data->actual_h - line_size, widget_data->actual_w, line_size };
				WzRect right0 = (WzRect){ 0 + widget_data->actual_w - line_size, 0, line_size, widget_data->actual_h };
				WzRect bottom1 = (WzRect){ 0 + 1 * line_size, 0 + widget_data->actual_h - 2 * line_size, widget_data->actual_w - 3 * line_size, line_size };
				WzRect right1 = (WzRect){ 0 + widget_data->actual_w - 2 * line_size, 0 + 1 * line_size, line_size, widget_data->actual_h - 2 * line_size };

				wzrd_is_rect_inside_rect(top0, (WzRect) { widget_data->actual_x, widget_data->actual_y, widget_data->actual_w, widget_data->actual_h });
				wzrd_is_rect_inside_rect(left0, (WzRect) { widget_data->actual_x, widget_data->actual_y, widget_data->actual_w, widget_data->actual_h });
				wzrd_is_rect_inside_rect(top1, (WzRect) { widget_data->actual_x, widget_data->actual_y, widget_data->actual_w, widget_data->actual_h });
				wzrd_is_rect_inside_rect(left1, (WzRect) { widget_data->actual_x, widget_data->actual_y, widget_data->actual_w, widget_data->actual_h });
				wzrd_is_rect_inside_rect(bottom0, (WzRect) { widget_data->actual_x, widget_data->actual_y, widget_data->actual_w, widget_data->actual_h });
				wzrd_is_rect_inside_rect(right0, (WzRect) { widget_data->actual_x, widget_data->actual_y, widget_data->actual_w, widget_data->actual_h });
				wzrd_is_rect_inside_rect(bottom1, (WzRect) { widget_data->actual_x, widget_data->actual_y, widget_data->actual_w, widget_data->actual_h });
				wzrd_is_rect_inside_rect(right1, (WzRect) { widget_data->actual_x, widget_data->actual_y, widget_data->actual_w, widget_data->actual_h });

				if (widget_data->border_type == WZ_BORDER_TYPE_DEFAULT) {
					// Draw top and left lines
					wz_widget_add_rect_new(widget, top0.x, top0.y, top0.w, top0.h, EGUI_WHITE2);
					wz_widget_add_rect_new(widget, left0.x, left0.y, left0.w, left0.h, EGUI_WHITE2);
					wz_widget_add_rect_new(widget, top1.x, top1.y, top1.w, top1.h, WZ_LIGHTGRAY);
					wz_widget_add_rect_new(widget, left1.x, left1.y, left1.w, left1.h, WZ_LIGHTGRAY);

					// Draw bottom and right lines
					wz_widget_add_rect_new(widget, bottom0.x, bottom0.y, bottom0.w, bottom0.h, WZ_BLACK);
					wz_widget_add_rect_new(widget, right0.x, right0.y, right0.w, right0.h, WZ_BLACK);
					wz_widget_add_rect_new(widget, bottom1.x, bottom1.y, bottom1.w, bottom1.h, EGUI_GRAY);
					wz_widget_add_rect_new(widget, right1.x, right1.y, right1.w, right1.h, EGUI_GRAY);
				}
				else if (widget_data->border_type == WZ_BORDER_TYPE_RED) {
					// Draw top and left lines
					wz_widget_add_rect_new(widget, top0.x, top0.y, top0.w, top0.h, WZ_RED);
					wz_widget_add_rect_new(widget, left0.x, left0.y, left0.w, left0.h, WZ_RED);
					wz_widget_add_rect_new(widget, top1.x, top1.y, top1.w, top1.h, WZ_RED);
					wz_widget_add_rect_new(widget, left1.x, left1.y, left1.w, left1.h, WZ_RED);

					// Draw bottom and right lines
					wz_widget_add_rect_new(widget, bottom0.x, bottom0.y, bottom0.w, bottom0.h, WZ_RED);
					wz_widget_add_rect_new(widget, right0.x, right0.y, right0.w, right0.h, WZ_RED);
					wz_widget_add_rect_new(widget, bottom1.x, bottom1.y, bottom1.w, bottom1.h, WZ_RED);
					wz_widget_add_rect_new(widget, right1.x, right1.y, right1.w, right1.h, WZ_RED);
				}
				else if (widget_data->border_type == WZ_BORDER_TYPE_TAB) {
					right0.y += 2;
					right0.h -= 2;
					left0.y += 2;
					left0.h -= 2;
					...
					wz_widget_add_rect_new(widget, right0.x, right0.y, right0.w, right0.h, WZ_BLACK);
					wz_widget_add_rect_new(widget, right1.x, right1.y, right1.w, right1.h, EGUI_GRAY);
					wz_widget_add_rect_new(widget, bottom1.x, bottom1.y, bottom1.w, bottom1.h, 0);
					wz_widget_add_rect_new(widget, bottom0.x, bottom0.y, bottom0.w, bottom0.h, 0);
					...
					wz_widget_add_rect_new(widget, top1.x, top1.y, top1.w, top1.h, EGUI_GRAY);
					wz_widget_add_rect_new(widget, left1.x, left1.y, left1.w, left1.h, EGUI_GRAY);

					// Draw bottom and right lines
					wz_widget_add_rect_new(widget, bottom0.x, bottom0.y, bottom0.w, bottom0.h, EGUI_WHITE2);
					wz_widget_add_rect_new(widget, right0.x, right0.y, right0.w, right0.h, EGUI_WHITE2);
					...
					wz_widget_add_rect_new(widget, top1.x, top1.y, top1.w, top1.h, WZ_BLACK);
					wz_widget_add_rect_new(widget, left1.x, left1.y, left1.w, left1.h, WZ_BLACK);
					...
					wz_widget_add_rect_new(widget, bottom0.x, bottom0.y, bottom0.w, bottom0.h, WZ_BLACK);
					wz_widget_add_rect_new(widget, right0.x, right0.y, right0.w, right0.h, WZ_BLACK);
					wz_widget_add_rect_new(widget, bottom1.x, bottom1.y, bottom1.w, bottom1.h, WZ_BLACK);
					wz_widget_add_rect_new(widget, right1.x, right1.y, right1.w, right1.h, WZ_BLACK);
#endif
				}
				else if (widget_data->border_type == WZ_BORDER_TYPE_BOTTOM_LINE) {
					wz_widget_add_rect_new(widget, bottom0.x, bottom0.y, bottom0.w, bottom0.h, EGUI_WHITE2);
					wz_widget_add_rect_new(widget, bottom1.x, bottom1.y, bottom1.w, bottom1.h, EGUI_GRAY);
				}
				else if (widget_data->border_type == BorderType_LeftLine) {
					wz_widget_add_rect_new(widget, left0.x, left0.y, left0.w, left0.h, EGUI_GRAY);
					wz_widget_add_rect_new(widget, left1.x, left1.y, left1.w, left1.h, EGUI_WHITE2);
				}
			}
		}
		...
		for (int j = 0; j < widget.items_count; ++j)
		{
			...
			{
				float w, h;
				wz->get_string_size(item.val.str.str, 0, strlen(item.val.str.str), & w, &h);

				item_dest_rect.w = (int)w;
				item_dest_rect.h = (int)h;

				if (widget.text_alignment == WZ_TEXT_ALIGNMENT_LEFT)
				{
					item_dest_rect.x = widget.actual_x;
					item_dest_rect.y = widget.actual_y;
				}
			...
			if (item.type == ItemType_Rect)
			{
				...
				wz_assert(temp_buffer[temp_buffer_count - 1].dest_rect.w > 0);
				wz_assert(temp_buffer[temp_buffer_count - 1].dest_rect.h > 0);
			}
			...
			// Apply item margins
			temp_buffer[temp_buffer_count - 1].dest_rect.x += item.margin_left;
			temp_buffer[temp_buffer_count - 1].dest_rect.y += item.margin_top;
		...
		// Draw clip area
		{
			...
			// Apply transformations to commands in temp buffer
			{
				...
				for (int cmd_idx = 0; cmd_idx < temp_buffer_count; ++cmd_idx)
				{
					...
					// Scale
					cmd->dest_rect.w *= current_widget->world_scale[0];
					cmd->dest_rect.h *= current_widget->world_scale[1];

					// Rotate position
					float x = cmd->dest_rect.x;
					float y = cmd->dest_rect.y;
					cmd->dest_rect.x = x * cos(current_widget->world_rotation) -
						y * sin(current_widget->world_rotation);
					cmd->dest_rect.y = x * sin(current_widget->world_rotation) +
						y * cos(current_widget->world_rotation);

					// Translate
					cmd->dest_rect.x += current_widget->world_pos[0];
					cmd->dest_rect.y += current_widget->world_pos[1];

#if 1
					// Apply camera offset (for all widgets)
					cmd->dest_rect.x -= wz->camera_x;
					cmd->dest_rect.y -= wz->camera_y;

					// Apply zoom (for all widgets)
					cmd->dest_rect.x *= (1 + wz->zoom_factor);
					cmd->dest_rect.y *= (1 + wz->zoom_factor);
					cmd->dest_rect.w *= (1 + wz->zoom_factor);
					cmd->dest_rect.h *= (1 + wz->zoom_factor);
#endif
				}
			}
		}

		// Copy all commands from temp buffer to main buffer
		for (int cmd_idx = 0; cmd_idx < temp_buffer_count; ++cmd_idx)
		{
			wz_assert(buffer->count < MAX_NUM_DRAW_COMMANDS - 1);
			buffer->commands[buffer->count] = temp_buffer[cmd_idx];
			buffer->commands[buffer->count].source = widget.source;
			buffer->count++;
		}
...
void wz_do_layout_refactor_me()
{
	...
	wz_do_layout(1, wz->widgets, wz->rects, MAX_NUM_WIDGETS, &layout_failed);
}
...
WzWidget wz_sized_box(WzWidget parent, unsigned w, unsigned h)
{
	...
	wz_widget_set_tight_constraints(result, w, h);
	...
	WzWidget* panels, unsigned* current_tab)
{
	...
	wz_widget_set_tight_constraints(panel, 100, 100);
...
#define MAX_NUM_TABS 32
	...
	{
		bool b = false;
		WzWidget tab = wz_command_button(tab_names[i], &b, tabs_panel);
...
void wz_end()
{
	...
	//for (int pointer = 1; pointer < stack_size; )
	while (stack_size)
	{
		WzWidget widget = indices_stack[stack_size - 1];
		wz->boxes_indices[wz->boxes_indices_count++] = widget;
		stack_size--;

		WzWidgetData* data = wz_widget_get(widget);

		for (unsigned i = 0; i < data->children_count; ++i)
		{
			indices_stack[stack_size++] = data->children[i];
		}
	}

	qsort(wz->boxes_indices, MAX_NUM_WIDGETS, sizeof(WzWidget), wzrd_compare_boxes);
	...
	// Debug string
	{
		...
		{
			wz->get_string_size(line_str, &w, &h);
		}
...
{
	float w = 0, h = 0;
	wz->get_string_size(str.str, 0, strlen(str.str), & w, &h);

	WzWidget parent = wz_widget_raw(handle, file, line);
	wz_widget_set_tight_constraints(parent, (int)w, (int)h);
...
static float wz_textedit_get_width(WzInputState* obj, int n, int i) {
	return 8.0f; // Fixed width per character
}

static void wz_textedit_delete(WzInputState* obj, int pos, int num) {
	memmove(&obj->buffer[pos], &obj->buffer[pos + num], obj->length - pos - num);
	obj->length -= num;
...
float wz_get_font_width(WzInputState *state, int line_start, int char_index)
{
	...
	float w, h;
	wz->get_string_size(c, 0, 1, &w, &h);

	return w;
}

static int wz_textedit_insert(WzInputState* obj, int pos, char* text, int num) {
	if (obj->length + num > obj->max_length - 1)
		num = obj->max_length - 1 - obj->length;

	memmove(&obj->buffer[pos + num], &obj->buffer[pos], obj->length - pos);
	memcpy(&obj->buffer[pos], text, num);
	obj->length += num;
	obj->buffer[obj->length] = 0;
	return num;
}
	...
	WzWidget parent, const char* file, unsigned int line)
{
	...
	// Initialization code when clicked
	
	if (!is_active)
	{
		is_active = true;

		stb_textedit_initialize_state(&wz->input_state.textedit_state, 1);
	}

	if (is_active)
	{
		// Process keyboard - use STB constants directly
		for (int i = 0; i < 512; ++i) {
			WzState state = wz->keyboard.keys[i];

			if (state == WZ_ACTIVATING || state == WZ_ACTIVE) {
				bool handled = false;
				int stb_key = 0;

				// Map your key codes to STB keys
				switch (i) {
				case '\b':
				{
				...
				case WZ_KEY_DELETE:
					...
					break;
				case WZ_KEY_LEFT: 
					stb_key = STB_TEXTEDIT_K_LEFT;
					handled = true;
					break;
				case WZ_KEY_RIGHT:
					stb_key = STB_TEXTEDIT_K_RIGHT;
					handled = true;
					break;
				case WZ_KEY_UP:
				...
				case WZ_KEY_END:
					...
					break;
				case 1: // Ctrl+A
					stb_textedit_key(&wz->input_state, &wz->input_state.textedit_state,
				...
				case 25: // Ctrl+Y
					...
					break;
				default:
					if (state == WZ_ACTIVATING && (isprint(i) || i == ' '))
					{
						char c = (char)i;
						stb_textedit_paste(&wz->input_state.buffer, &wz->input_state.textedit_state,
							&c, 1);
						handled = true;
					}
					break;
				...
				if (handled && stb_key)
				{
					stb_textedit_key(&wz->input_state, &wz->input_state.textedit_state, stb_key);
				}
			}
		}

		// Draw cursor
		if ((int)(wz->input_box_timer / 500) % 2 == 0) 
		{
			float w, h;
			wz->get_string_size(wz->input_state.buffer,
				0, wz->input_state.textedit_state.cursor,
				&w, &h);
			wz_widget_add_rect_new(widget, (int)w, 2, 1, 16, WZ_BLACK);
		}
		
		// Draw selection
		if (wz->input_state.textedit_state.select_start !=
			wz->input_state.textedit_state.select_end)
		{
			int sel_start = wz->input_state.textedit_state.select_start;
			int sel_end = wz->input_state.textedit_state.select_end;
			...
			int sel_x = sel_start * 8;
			int sel_w = (sel_end - sel_start) * 8;
			wz_widget_add_rect_new(widget, sel_x, 2, sel_w, 16, 0x6699FFAA);
		}
	}

	wz_widget_add_text(widget, wz_str_create_slice(wz->input_state.buffer, 0, wz->input_state.length));
...
WzWidget wz_command_button_raw(WzStr str, bool* released,
	WzWidget parent, const char* file_name, unsigned int line)
{
	WzWidget button = wz_label_raw(parent, str, file_name, line);
	wz_widget_set_border(button, WZ_BORDER_TYPE_DEFAULT);
	wz_widget_set_text_alignment(button, WZ_TEXT_ALIGNMENT_CENTER);

	wz_command_button_run(button, released);
...
void wz_frame(WzWidget parent, unsigned w, unsigned h, WzStr str)
{
	...
	const int offset = 12;
	wz_widget_set_tight_constraints(widget, w, h);
...
{
	wz_widget_set_tight_constraints(c, w, h);
}
